...
export class ProductComponent implements OnInit {
    ...
    private editProduct(event) {
        try {
            this.message.text = "";
            this.isNewProduct = false;
            this.resetValues();
            let editedProduct = Object.assign({}, this.lstProducts.find(x => x.id == event.productId));
            ...
            this.productModalRef = event.modalRef;
        }
        catch (e) {
            HandleError.handle(e);
        }
    ...
    public openModal(template: TemplateRef<any>) {
        try {
            this.productModalRef = this.modalServiceRef.show(template, { class: 'modal-lg' });
        }
        catch (e) {
            HandleError.handle(e);
        }
